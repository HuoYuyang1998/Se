...
###  Using selective conditional conformal prediction to give prediction intervals after selection
  ...
  if(Thresholding_type=="cons"|Thresholding_type=="exch"|!exchangeable_adapted){
    ...
    cali_j<-R_cal[which(T_cal>=T_ordered[max(kappa-1,0)])]
...
{# computing the threshold by selection rule T-pos(b_0,beta)
