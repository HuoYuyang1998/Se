...
### Setting the data generating functions associated with the algorithms
setMethod("DataGen","SVM",function(obj,X,beta,sigma){
  uX=X[,1]*X[,2]+X[,3]-2*exp(X[,4]+1)
  Y=uX+rnorm(dim(X)[1],0,1)
  #Y=uX+rt(dim(X)[1],2)  ### reproducing Table S3
...
#Thresholding_type_array=c("test","POS","top","clu")
FDR_level=0.2 ### POS selection rule
K=60  ### Tok-K selection rule
Thresholding_quantile=0.3 ### the quantile level used in test selection rule
...
Result<-foreach(iter=1:ns,.combine="rbind",.packages = c('MASS',"randomForest","kernlab"))%dopar% {
  ...
  for (algo in algo_array) {
    ...
    b_0=if(algo@name=="OLS"){-1}else if(algo@name=="SVM"){-8}else if(algo@name=="RF"){-1.2}
    ...
    for (Thresholding_type in Thresholding_type_array) {
      ...
      Selection_rule<-list(Thresholding_type=Thresholding_type,K=K,FDR_level=FDR_level,b_0=b_0,Thresholding_quantile=Thresholding_quantile)
      
      L<-Selection_thresholding(Selection_rule,T_cal,T_test,Y_cal)
      Select_test<-which(T_test<=L)
      
      
      #Result_SCOP<-SCOP(R_cal,R_test,T_cal,T_test,Select_test,L,alpha,Selection_rule,exchangeable_adapted=FALSE)  ### reproducing Table S4
...
Result$Setting[Result$Setting=="RF"]<-"Scenario C"
...
Table<-Result%>%
  group_by(method,Thresholding_type,Setting,n,m)%>%
